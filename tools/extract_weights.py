...
def build_expert_index(header_cache, model_path, num_experts):
    """Build expert_index.json metadata directly from safetensors headers."""
    ...
    for filename in sorted(header_cache.keys()):
        ...
        for tensor_name, meta in header.items():
            ...
            layer_idx, component, sub = match.groups()
            tensor_offsets = meta['data_offsets']
            total_size = tensor_offsets[1] - tensor_offsets[0]
            if total_size % num_experts != 0:
                raise ValueError(
                    f"Expert tensor {tensor_name} has total size {total_size}, "
                    f"not divisible by {num_experts}"
                )

            expert_stride = total_size // num_experts
            component_key = f"{component}.{sub}"
            expert_reads[layer_idx][component_key] = {
    ...
    ordered_layers = {}
    for layer_idx in sorted(expert_reads.keys(), key=int):
        missing = [name for name in component_order if name not in expert_reads[layer_idx]]
        if missing:
            raise ValueError(f"Layer {layer_idx} missing expert components: {missing}")
        ordered_layers[layer_idx] = {
            name: expert_reads[layer_idx][name]
            for name in component_order
        }

    return {
        "model_path": str(model_path),
        "expert_reads": ordered_layers,
    }
...
def main():
    ...
    expert_index_path = output_dir / 'expert_index.json'
